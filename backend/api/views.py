...
class AlunoViewSet(viewsets.ModelViewSet):
    queryset = Aluno.objects.all() # O Django pede isso por padrão, mas o get_queryset abaixo vai sobrescrever!
    serializer_class = AlunoSerializer
    permission_classes = [permissions.IsAuthenticated]
    
    # 1. A MÁGICA DA LISTAGEM (GET)
    # Quando o front-end pedir a lista, o backend vai rodar isso aqui e devolver só os alunos desse professor
    def get_queryset(self):
        return Aluno.objects.filter(professor=self.request.user.perfil)

    # 2. A MÁGICA DA CRIAÇÃO (POST)
    # Quando o front-end enviar os dados do aluno novo, o backend intercepta e carimba o dono antes de salvar
