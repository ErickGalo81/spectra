...
interface Plano {
  ...
  social: number;
  motor: number;
  aluno?: number; // Assumindo que o backend retorna o ID do aluno
}
    ...
    const carregarPlano = async () => {
      ...
      try {
        ...
        axios.get('http://localhost:8000/api/me/', config)
          ...
          .catch(() => setUsuario({ nome: "Professor", cargo: "Docente" }));

        // Busca o PEI específico pelo ID da URL
        const resPei = await axios.get(`http://localhost:8000/api/peis/${id}/`, config);
        setPlano(resPei.data);

        // Se o PEI tiver o ID do aluno, busca o nome dele para exibir
        ...
        alert("Não foi possível carregar este plano. Ele pode ter sido excluído ou você não tem permissão.");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center">
        ...
        <p className="text-slate-500 font-bold uppercase tracking-tighter">Carregando Prontuário...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col items-center">
      ...
      <main className="w-full max-w-4xl px-4 pb-12">
        <div className="bg-white rounded-[40px] px-8 md:px-12 py-10 shadow-sm border border-slate-200 relative">
          ...
          {/* Card do Aluno e Título do Plano */}
              ...
              {plano.protocolo_crise && plano.protocolo_crise.length > 0 ? (
                <div className="space-y-3">
                  {plano.protocolo_crise.map((passo, idx) => (
                    <div key={idx} className="flex items-start gap-4 bg-red-50 p-5 rounded-2xl border border-red-100">
