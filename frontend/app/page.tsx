...
export default function LoginPage() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // TRATAMENTO DE ERRO (UX APLICADA)
      if (err.response && err.response.status === 401) {
        // Erro clássico de autenticação (Senha errada ou usuário não existe)
        setErro('⚠️ E-mail ou senha incorretos. Tente novamente.');
      } else if (err.response && err.response.data) {
        // Se o Django mandou um erro diferente
        setErro('⚠️ Credenciais inválidas ou usuário não encontrado.');
      } else {
        // Se a requisição sequer bateu no Django (Backend desligado ou erro de CORS)
        setErro('⚠️ Erro de conexão. Verifique se o servidor Django está rodando na porta 8000.');
      }
    } finally {
      setLoading(false);
    }
