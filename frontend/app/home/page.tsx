...
export default function HomePage() {
  ...
  useEffect(() => {
    const carregarDados = async () => {
      ...
      try {
        // 👇 AGORA ESTAMOS BUSCANDO OS PEIS TAMBÉM!
        ...
        // 👇 JUNTANDO OS ALUNOS COM SEUS RESPECTIVOS PLANOS
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="bg-slate-100 rounded-[32px] p-10 mb-10 text-white flex flex-col md:flex-row justify-between items-center shadow-2xl relative overflow-hidden">
          <div className="relative z-10">
            <h1 className="text-indigo-600 text-4xl font-bold tracking-tight mb-2">Bem-vindo, {usuario.nome.split(" ")[0]}!</h1>
            <p className="text-indigo-400 font-medium text-lg">Você tem <span className="text-indigo-600 font-bold">{alunos.length} alunos</span> ativos no sistema.</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {alunos.map((aluno) => {
              // CALCULA A EVOLUÇÃO COM BASE NO PLANO ATIVO!
                  ...
                  <div className="bg-white p-8 rounded-[32px] border-2 border-transparent shadow-sm group-hover:shadow-2xl group-hover:border-indigo-100 transition-all duration-300 flex flex-col h-full border-slate-100">
                    ...
                    <div className="space-y-5 mt-auto">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase text-slate-400 tracking-wider">
                        <span>Evolução</span>
                        <span className="text-indigo-600">{evolucaoMedia.toFixed(0)}%</span>
                      </div>
